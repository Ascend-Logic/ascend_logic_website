...
function getCaseStudyById(id: number): DetailedCaseStudy | undefined {
  const caseStudies: DetailedCaseStudy[] = [
    ...
    },
    {
      ...
      description: '契約書や領収書などの紙ベース帳票をPDFとしてアップロードすると、AI-OCRエンジンが文書内のテキスト・表組み・手書き文字を自動認識し、構造化データを抽出。確認・編集後にワンクリックでExcelへ自動転記でき、従来の手入力作業を大幅に削減するソリューションを構築しました。',
      company: '不動産管理会社',
      industry: '不動産・管理',
      solution: 'プロセス自動化',
      results: ['手入力工数を80%以上削減', '複数ファイルの一括処理に対応', 'データ抽出精度の大幅向上', '契約書管理業務の効率化を実現'],
      imagePath: '/images/case-study-aiocr.jpg',
      challenge: '不動産管理業務では、契約書・領収書・各種届出書類など膨大な紙ベースの帳票を日常的に処理する必要がありました。物件名・号室・契約者名・日付などの情報を手作業でExcelに転記しており、入力ミスや作業工数の増大が慢性的な課題となっていました。',
      approach: 'AI-OCRエンジンを活用し、PDFアップロードからデータ抽出・確認・Excel出力までを4ステップのワークフロー（ファイルアップロード → OCR処理 → データ確認 → エクセルダウンロード）で設計。原本PDFと抽出結果を並べて確認・編集できるプレビュー画面を実装し、複数ファイルの一括処理にも対応しました。',
